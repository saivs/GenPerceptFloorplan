...
def calculate_class_weights(dataset, num_classes):
    ...
    Args:
        dataset: Dataset object with "seg_class_indices" in its items
        num_classes (int): Number of segmentation classes
    ...
    for i in range(len(dataset)):
        data = dataset[i]
        if "seg_class_indices" in data:
            indices = data["seg_class_indices"]
            for c in range(num_classes):
                class_counts[c] += (indices == c).sum().item()
    ...
    class_frequencies = torch.where(
        class_frequencies > 0,
        class_frequencies,
        torch.ones_like(class_frequencies)
    )
